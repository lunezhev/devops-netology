...
resource "yandex_storage_bucket" "lunezhev" {
...
EOT
  ...
  depends_on = [yandex_storage_bucket.lunezhev]
}

resource "null_resource" "example" {
  provisioner "local-exec" {
    command = "yc iam key create --service-account-id ajecdb2h49krqb7h0vmv --folder-name test --output ./keys/key.json"
  }
  depends_on = [yandex_resourcemanager_folder_iam_member.tf-sa-editor]
}